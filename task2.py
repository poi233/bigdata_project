...
def check_semantic_type(x):
    ...
    # Phone Number
    if is_phone(x):
        predict_types.append(('phone_number', 1))
    # Address
    # Street Name
    # City
    if is_city(x):
        predict_types.append(('city', 1))
    # Neighborhood
    # LAT/LON coordinates
    # Zip code
    if is_zip(x):
        predict_types.append(('zip_code', 1))
    # Borough
    if is_borough(x):
        predict_types.append(('borough', 1))
    # School name
    # Color
    # Car Make
    # City Agency
    ...
    # websites
    if is_website(x):
        predict_types.append(('website', 1))
...
def is_zip(x):
    pattern = re.compile(r'^[\d]{5,5}$')
    return re.match(pattern, x)


def is_city(x):
    return x.lower() in external.cities or external.counties


def is_borough(x):
    return x.lower() in external.borough
        ...
        for column in files[file]:
            ...
            for col in opendata_df.columns:
                if re.match(pattern, col):
                    ...
                    all_types = column_df.rdd.flatMap(check_semantic_type).reduceByKey(lambda a, b: a + b).sortBy(lambda x: -x[1]).collect()
                    print(all_types)
                    print("%s %s finish" % (file, col))
        print("%s finish" % file)
